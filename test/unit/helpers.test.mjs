...
test('assertGetRandomValuesAvailable throws when getRandomValues missing', () => {
  setCrypto({ subtle: {} })
  expectCode(
    () => assertGetRandomValuesAvailable('test'),
...
test('assertAesGcmIv96 accepts 12-byte iv and rejects others', () => {
  ...
  expectCode(
    () => assertAesGcmIv96(new Uint8Array(11)),
...
test('assertRawAesGcm256Bytes enforces 32 bytes', () => {
  ...
  expectCode(
    () => assertRawAesGcm256Bytes(new Uint8Array(31)),
...
test('assertRawHmac256Bytes enforces 32 bytes', () => {
  ...
  expectCode(
    () => assertRawHmac256Bytes(new Uint8Array(31)),
...
test('assertAesGcm256Key rejects invalid JWKs', () => {
  ...
  expectCode(
    () => assertAesGcm256Key(aesJwk({ kty: 'RSA' })),
    'AES_GCM_KEY_EXPECTED'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ alg: 'A128GCM' })),
    'AES_GCM_ALG_INVALID'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ use: 'sig' })),
    'AES_GCM_USE_INVALID'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ key_ops: 'encrypt' })),
    'AES_GCM_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ key_ops: ['encrypt', 'sign'] })),
    'AES_GCM_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ key_ops: ['encrypt'] })),
  ...
  expectCode(
    () => assertAesGcm256Key(aesJwk({ key_ops: ['decrypt'] })),
    'AES_GCM_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ k: undefined })),
    'AES_GCM_KEY_EXPECTED'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ k: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertAesGcm256Key(aesJwk({ k: b64_31 })),
...
test('assertHmacSha256Key rejects invalid JWKs', () => {
  ...
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ kty: 'RSA' })),
    'HMAC_KEY_EXPECTED'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ alg: 'HS384' })),
    'HMAC_ALG_INVALID'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ use: 'enc' })),
    'HMAC_USE_INVALID'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ key_ops: 'sign' })),
    'HMAC_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ key_ops: ['sign', 'encrypt'] })),
    'HMAC_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ key_ops: ['sign'] })),
  ...
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ key_ops: ['verify'] })),
    'HMAC_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ k: undefined })),
    'HMAC_KEY_EXPECTED'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ k: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertHmacSha256Key(hmacJwk({ k: b64_31 })),
...
test('assertEd25519PrivateKey rejects invalid JWKs', () => {
  expectCode(
    () => assertEd25519PrivateKey(null),
    'ED25519_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ kty: 'EC' })),
    'ED25519_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ crv: 'X25519' })),
    'ED25519_CURVE_INVALID'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ alg: 'Ed448' })),
    'ED25519_ALG_INVALID'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ use: 'enc' })),
    'ED25519_USE_INVALID'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ key_ops: 'sign' })),
    'ED25519_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ key_ops: ['verify'] })),
    'ED25519_KEY_OPS_INVALID'
  )
  expectCode(
    () =>
      assertEd25519PrivateKey(edPrivateJwk({ key_ops: ['sign', 'wrapKey'] })),
    'ED25519_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ x: undefined })),
    'ED25519_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ d: undefined })),
    'ED25519_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ x: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ x: b64_31 })),
    'ED25519_KEY_SIZE_INVALID'
  )
  expectCode(
    () => assertEd25519PrivateKey(edPrivateJwk({ d: b64_31 })),
    'ED25519_KEY_SIZE_INVALID'
  )
})
...
test('assertEd25519PublicKey rejects invalid JWKs', () => {
  ...
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ kty: 'EC' })),
    'ED25519_PUBLIC_KEY_EXPECTED'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ crv: 'X25519' })),
    'ED25519_CURVE_INVALID'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ alg: 'Ed448' })),
    'ED25519_ALG_INVALID'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ use: 'enc' })),
    'ED25519_USE_INVALID'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ key_ops: 'verify' })),
    'ED25519_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ key_ops: ['sign'] })),
    'ED25519_KEY_OPS_INVALID'
  )
  expectCode(
    () =>
      assertEd25519PublicKey(edPublicJwk({ key_ops: ['verify', 'wrapKey'] })),
    'ED25519_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ x: undefined })),
    'ED25519_PUBLIC_KEY_EXPECTED'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ d: b64_32 })),
    'ED25519_PUBLIC_KEY_EXPECTED'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ x: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertEd25519PublicKey(edPublicJwk({ x: b64_31 })),
    'ED25519_KEY_SIZE_INVALID'
  )
})
...
test('assertRsaOaep4096PublicKey rejects invalid JWKs', () => {
  expectCode(
    () => assertRsaOaep4096PublicKey(null),
    'RSA_OAEP_PUBLIC_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ kty: 'EC' })),
    'RSA_OAEP_PUBLIC_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ alg: 'RSA-OAEP' })),
    'RSA_OAEP_ALG_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ use: 'sig' })),
    'RSA_OAEP_USE_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ key_ops: 'wrapKey' })),
    'RSA_OAEP_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ key_ops: ['encrypt'] })),
    'RSA_OAEP_KEY_OPS_INVALID'
  )
  expectCode(
    () =>
      assertRsaOaep4096PublicKey(
        rsaPublicJwk({ key_ops: ['wrapKey', 'sign'] })
      ),
    'RSA_OAEP_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ n: undefined })),
    'RSA_OAEP_PUBLIC_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ d: b64_32 })),
    'RSA_OAEP_PUBLIC_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ n: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ n: rsaN_short })),
    'RSA_OAEP_MODULUS_LENGTH_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ e: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PublicKey(rsaPublicJwk({ e: rsaE_bad })),
...
test('assertRsaOaep4096PrivateKey rejects invalid JWKs', () => {
  expectCode(
    () => assertRsaOaep4096PrivateKey(null),
    'RSA_OAEP_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ kty: 'EC' })),
    'RSA_OAEP_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ alg: 'RSA-OAEP' })),
    'RSA_OAEP_ALG_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ use: 'sig' })),
    'RSA_OAEP_USE_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ key_ops: 'unwrapKey' })),
    'RSA_OAEP_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ key_ops: ['decrypt'] })),
    'RSA_OAEP_KEY_OPS_INVALID'
  )
  expectCode(
    () =>
      assertRsaOaep4096PrivateKey(
        rsaPrivateJwk({ key_ops: ['unwrapKey', 'sign'] })
      ),
    'RSA_OAEP_KEY_OPS_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ n: undefined })),
    'RSA_OAEP_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ d: undefined })),
    'RSA_OAEP_PRIVATE_KEY_EXPECTED'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ n: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ n: rsaN_short })),
    'RSA_OAEP_MODULUS_LENGTH_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ e: b64_invalid_len })),
    'BASE64URL_INVALID'
  )
  expectCode(
    () => assertRsaOaep4096PrivateKey(rsaPrivateJwk({ e: rsaE_bad })),
    'RSA_OAEP_EXPONENT_INVALID'
  )
})
