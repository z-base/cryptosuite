  ...
  await withCrypto(
    ...
    async () => {
      await expectCodeAsync(
        () => deriveCipherKey(new Uint8Array(32)),
        'AES_GCM_UNSUPPORTED'
      )
    }
  )
...
test('CipherAgent maps importKey failure', async () => {
  await withCrypto(
    ...
    async () => {
      ...
      await expectCodeAsync(
        () => agent.encrypt(new Uint8Array([1])),
        'AES_GCM_UNSUPPORTED'
      )
    }
  )
...
test('deriveHMACKey maps unsupported HMAC', async () => {
  await withCrypto(
    ...
    async () => {
      await expectCodeAsync(
        () => deriveHMACKey(new Uint8Array(32)),
...
test('VerifyAgent maps importKey failure', async () => {
  await withCrypto(
    ...
    async () => {
      ...
      await expectCodeAsync(
        () => agent.verify(new Uint8Array([1]), new ArrayBuffer(1)),
        'ED25519_UNSUPPORTED'
      )
    }
  )
...
test('UnwrapAgent maps RSA importKey failure', async () => {
  await withCrypto(
    ...
    async () => {
      ...
      await expectCodeAsync(
        () => agent.unwrap(new ArrayBuffer(1)),
  ...
  await withCrypto(
    buildCrypto({
      subtle: {
        ...
        exportKey: async (_format, key) =>
          key === privateKey ? signJwk : verifyJwk,
      },
    }),
    async () => {
  ...
  await withCrypto(
    buildCrypto({
      subtle: {
        generateKey: async () => ({ publicKey, privateKey }),
        exportKey: async (_format, key) =>
          key === publicKey ? wrapJwk : unwrapJwk,
      },
    }),
    async () => {
