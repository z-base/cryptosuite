...
test("Ed25519 sign/verify or correct error", async ({ page }) => {
  await page.goto("/");
  const result = await evaluate(page, async () => {
    const { generateVerificationPair, SignAgent, VerifyAgent } = await import(
      "/dist/index.js",
    );
  ...
  if (!result.ok) {
    expect(["ED25519_UNSUPPORTED", "ED25519_ALG_INVALID"]).toContain(result.code);
  } else {
    expect(result.verified).toBe(true);
  }
});
...
test("OID generate/derive/validate", async ({ page }) => {
  await page.goto("/");
  const result = await evaluate(page, async () => {
    const { generateOID, deriveOID, validateOID } = await import("/dist/index.js");
